...
function useVoice({ onTranscript, onStart, onStop }) {
  ...
  const cbRef = useRef({ onTranscript, onStart, onStop });
  useEffect(() => { cbRef.current = { onTranscript, onStart, onStop }; });
...
export default function Workspace() {
  ...
  const sendVoiceExchange = useCallback(async () => {
    ...
    setInterim("");
    try {
      const res = await voiceExchange({ transcript: text, history: voiceHistoryRef.current });
